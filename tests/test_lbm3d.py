...
@pytest.fixture
def sim() -> LBM3DCPU:
    return LBM3DCPU(width=16, height=16, depth=16, viscosity=0.02)
...
class TestInit:
    ...
    def test_custom_grid(self, sim: LBM3DCPU) -> None:
        assert sim.width == 16
        assert sim.height == 16
        assert sim.depth == 16

    def test_omega_from_viscosity(self) -> None:
        s = LBM3DCPU(viscosity=0.1)
        expected = 1.0 / (3.0 * 0.1 + 0.5)
        assert s.omega == pytest.approx(expected)

    def test_initial_fields_not_nan(self, sim: LBM3DCPU) -> None:
    ...
    def test_initial_density_uniform(self, sim: LBM3DCPU) -> None:
        assert np.allclose(sim.rho, 1.0)

    def test_initial_velocity_uniform(self, sim: LBM3DCPU) -> None:
        assert np.allclose(sim.u, 0.1)
        assert np.allclose(sim.v, 0.0)
        assert np.allclose(sim.w_vel, 0.0)

    def test_f_shape(self, sim: LBM3DCPU) -> None:
        assert sim.f.shape == (19, 16, 16, 16)

    def test_omega_stable_assert(self) -> None:
        with pytest.raises(AssertionError):
            LBM3DCPU(viscosity=0.0)

    def test_grid_shapes(self, sim: LBM3DCPU) -> None:
...
class TestEquilibrium:
    def test_equilibrium_total_density(self, sim: LBM3DCPU) -> None:
        rho = np.ones((16, 16, 16))
        u = np.zeros((16, 16, 16))
        v = np.zeros((16, 16, 16))
        w = np.zeros((16, 16, 16))
        feq = sim.lattice.equilibrium(rho, u, v, w)
        rho_sum = np.sum(feq, axis=0)
        assert np.allclose(rho_sum, 1.0)

    def test_equilibrium_x_momentum(self, sim: LBM3DCPU) -> None:
        rho = np.ones((16, 16, 16))
        u_fld = np.full((16, 16, 16), 0.1)
        v_fld = np.zeros((16, 16, 16))
        w_fld = np.zeros((16, 16, 16))
        feq = sim.lattice.equilibrium(rho, u_fld, v_fld, w_fld)
        c = sim.lattice.cx[:, np.newaxis, np.newaxis, np.newaxis]
        u_comp = np.sum(feq * c, axis=0)
        assert np.allclose(u_comp, 0.1, atol=1e-12)

    def test_equilibrium_y_momentum(self, sim: LBM3DCPU) -> None:
        rho = np.ones((16, 16, 16))
        u_fld = np.zeros((16, 16, 16))
        v_fld = np.full((16, 16, 16), 0.05)
        w_fld = np.zeros((16, 16, 16))
        feq = sim.lattice.equilibrium(rho, u_fld, v_fld, w_fld)
        c = sim.lattice.cy[:, np.newaxis, np.newaxis, np.newaxis]
        v_comp = np.sum(feq * c, axis=0)
        assert np.allclose(v_comp, 0.05, atol=1e-12)

    def test_equilibrium_z_momentum(self, sim: LBM3DCPU) -> None:
        rho = np.ones((16, 16, 16))
        u_fld = np.zeros((16, 16, 16))
        v_fld = np.zeros((16, 16, 16))
        w_fld = np.full((16, 16, 16), 0.03)
        feq = sim.lattice.equilibrium(rho, u_fld, v_fld, w_fld)
        c = sim.lattice.cz[:, np.newaxis, np.newaxis, np.newaxis]
        w_comp = np.sum(feq * c, axis=0)
        assert np.allclose(w_comp, 0.03, atol=1e-12)


class TestCollision:
    def test_mass_conserved(self, sim: LBM3DCPU) -> None:
        mass_before = np.sum(sim.f)
        sim.collision()
        mass_after = np.sum(sim.f)
        assert mass_after == pytest.approx(mass_before, rel=1e-10)

    def test_output_shape(self, sim: LBM3DCPU) -> None:
        sim.collision()
        assert sim.f.shape == (19, 16, 16, 16)

    def test_momentum_conserved(self, sim: LBM3DCPU) -> None:
        c = sim.lattice.cx[:, np.newaxis, np.newaxis, np.newaxis]
...
class TestBoundaries:
    def test_inflow_sets_left_bc(self, sim: LBM3DCPU) -> None:
    ...
    def test_outflow_zero_gradient(self, sim: LBM3DCPU) -> None:
        sim.apply_outflow()
        for i in range(19):
...
class TestObstacles:
    def test_add_obstacle_sphere(self, sim: LBM3DCPU) -> None:
        sim.add_obstacle(8, 8, 8, radius=3)
        assert sim.obstacles[8, 8, 8]

    def test_clear_obstacles(self, sim: LBM3DCPU) -> None:
        sim.add_obstacle(8, 8, 8, radius=3)
        sim.clear_obstacles()
        assert not np.any(sim.obstacles)

    def test_obstacle_bounce_back(self, sim: LBM3DCPU) -> None:
...
class TestStep:
    def test_step_changes_state(self, sim: LBM3DCPU) -> None:
        f_before = sim.f.copy()
        sim.step()
        assert not np.allclose(sim.f, f_before)

    def test_multiple_steps(self, sim: LBM3DCPU) -> None:
        sim.run(10)
        assert sim.f.shape == (19, 16, 16, 16)

    def test_no_nan_after_steps(self, sim: LBM3DCPU) -> None:
        sim.run(50)
        assert not np.any(np.isnan(sim.f))
        assert not np.any(np.isnan(sim.rho))
        assert not np.any(np.isnan(sim.u))
...
class TestGetter:
    def test_get_density(self, sim: LBM3DCPU) -> None:
        d = sim.get_density()
        assert d.shape == (16, 16, 16)
        assert np.allclose(d, sim.rho)

    def test_get_velocity(self, sim: LBM3DCPU) -> None:
        v = sim.get_velocity()
    ...
    def test_get_smoke(self, sim: LBM3DCPU) -> None:
        s = sim.get_smoke()
...
class TestSmoke:
    def test_initial_smoke_zero(self, sim: LBM3DCPU) -> None:
        assert np.allclose(sim.smoke, 0.0)

    def test_add_emitter(self, sim: LBM3DCPU) -> None:
        sim.add_emitter(8, 8, 8, strength=0.05)
        assert len(sim.emitters) == 1
        assert sim.emitters[0] == (8, 8, 8, 0.05)

    def test_clear_emitters(self, sim: LBM3DCPU) -> None:
        sim.add_emitter(8, 8, 8, strength=0.05)
        sim.clear_emitters()
        assert len(sim.emitters) == 0

    def test_emitter_injects_smoke(self, sim: LBM3DCPU) -> None:
        sim.add_emitter(8, 8, 8, strength=0.1)
        sim.apply_emitters()
        assert sim.smoke[8, 8, 8] == pytest.approx(0.1)

    def test_smoke_advection_preserves_mass_at_zero_vel(self, sim: LBM3DCPU) -> None:
        sim.smoke[8, 8, 8] = 1.0
        sim.u[:] = 0.0
        sim.v[:] = 0.0
        sim.w_vel[:] = 0.0
    ...
    def test_smoke_advection_moves_downstream_x(self, sim: LBM3DCPU) -> None:
        sim.smoke[8, 8, 8] = 1.0
        sim.u[:] = 1.0
        sim.v[:] = 0.0
        sim.w_vel[:] = 0.0
    ...
    def test_smoke_advection_moves_downstream_z(self, sim: LBM3DCPU) -> None:
        sim.smoke[4, 8, 8] = 1.0
        sim.u[:] = 0.0
        sim.v[:] = 0.0
        sim.w_vel[:] = 1.0
    ...
    def test_decay_reduces_smoke(self, sim: LBM3DCPU) -> None:
        sim.smoke[8, 8, 8] = 1.0
        sim.decay_smoke()
        assert sim.smoke[8, 8, 8] == pytest.approx(0.999)

    def test_diffusion_spreads_smoke(self, sim: LBM3DCPU) -> None:
        sim.smoke[8, 8, 8] = 1.0
        sim.diffuse_smoke()
    ...
    def test_smoke_step_integration(self, sim: LBM3DCPU) -> None:
        sim.add_emitter(8, 8, 8, strength=0.1)
        sim.step()
        assert np.sum(sim.smoke) > 0
        assert np.all(sim.smoke >= 0)

    def test_initialize_clears_smoke(self, sim: LBM3DCPU) -> None:
        sim.smoke[8, 8, 8] = 1.0
