...
class TestInit:
    ...
    def test_initial_density_uniform(self, sim):
        assert np.allclose(sim.rho, 1.0)

    def test_initial_velocity_uniform(self, sim):
        assert np.allclose(sim.u, 0.1)
        assert np.allclose(sim.v, 0.0)


class TestEquilibrium:
    def test_equilibrium_total_density(self, sim):
        rho = np.ones((32, 32))
        u = np.zeros((32, 32))
        v = np.zeros((32, 32))
    ...
    def test_equilibrium_x_momentum(self, sim):
        rho = np.ones((32, 32))
...
class TestCollision:
    def test_mass_conserved(self, sim):
...
class TestBoundaries:
    def test_inflow_sets_left_bc(self, sim):
        sim.initialize(rho=1.0, u=0.0, v=0.0)
        sim.apply_inflow(u_inflow=0.15)
        u_left = (np.sum(sim.f * sim.cx[:, np.newaxis, np.newaxis], axis=0)
                  / np.sum(sim.f, axis=0))
        assert np.allclose(u_left[:, 0], 0.15, atol=1e-6)

    def test_outflow_zero_gradient(self, sim):
        sim.apply_outflow()
        for i in range(9):
            assert np.allclose(sim.f[i, :, -1], sim.f[i, :, -2])

    def test_walls_bounce_back_top(self, sim):
...
class TestObstacles:
    def test_add_obstacle(self, sim):
        sim.add_obstacle(16, 16, radius=5)
        assert sim.obstacles[16, 16]

    def test_clear_obstacles(self, sim):
        sim.add_obstacle(16, 16, radius=5)
        sim.clear_obstacles()
        assert not np.any(sim.obstacles)

    def test_obstacle_bounce_back(self, sim):
...
class TestGetter:
    def test_get_density(self, sim):
        d = sim.get_density()
        assert d.shape == (32, 32)
        assert np.allclose(d, sim.rho)

    def test_get_velocity(self, sim):
        v = sim.get_velocity()
